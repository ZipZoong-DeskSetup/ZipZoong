...
function ProductLikeButton({ itemId }: { itemId: number }) {
    ...
    // TODO: 주석풀기
    // useEffect(() => {
    //     axios.get<Hardware[]>('/favorite/product') 
    //         .then(response => {
    ...
    const handleLike = () => {
        if (isLiked) {
            // 좋아요를 취소하는 경우
            // TODO: 주석풀기
            // axios.delete(`/favorite/product/${productId}`)
            //     .then(() => {
            //         setIsLiked(false);
            //     })
            ...
            // 좋아요를 하는 경우
            // TODO: 주석풀기
            // axios.post('/favorite/product', { itemId })
            //     .then(response => {
            //         setIsLiked(true);
            //         setProductId(response.data.id); // response에 따라 변경
            //     })
    ...
    return (
        <>
            {isLiked ? (
                <button onClick={handleLike}><FaBookmark className={styles.like} /></button>
            ) : (
                <button onClick={handleLike}><FaRegBookmark className={styles.unlike} /></button>
            )}
        </>
    );
};
