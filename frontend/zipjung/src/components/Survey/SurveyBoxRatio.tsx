...
const SurveyBoxRatio = ({
  content,
  boxClick,
  design,
  isFocus,
}: {
  content: [string, string, 2 | 1 | 4 | 8][];
  boxClick: (focusing: (2 | 1 | 4 | 8)[]) => void;
  design: string;
  isFocus: (2 | 1 | 4 | 8 | -1)[];
}) => {
  const [value, setValue] = useState<(2 | 1 | 4 | 8 | -1)[]>(isFocus);
  ...
  const addValue = (index: 2 | 1 | 4 | 8): (2 | 1 | 4 | 8)[] | [] => {
    let tempValue = [...value];
    ...
    return tempValue as (2 | 1 | 4 | 8)[];
  };

  const handleBoxClick = (index: number) => {
    const newValue: (2 | 1 | 4 | 8)[] = addValue(content[index][2]);
    setValue(newValue);
    boxClick(newValue);
  };
