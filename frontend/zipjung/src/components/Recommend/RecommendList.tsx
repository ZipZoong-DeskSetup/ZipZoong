...
const RecommendList: React.FC<RecommendListProps> = ({ item }) => {
    ...
    const renderProduct = (product: KeyboardDetail | MouseDetail, title: string) => (
        <div className={styles.productDiv1}>
            <div className={styles.productDiv2}>
                <Image
                    src={title === "키보드" ? '/Images/keyboardIcon.png' : '/Images/mouseIcon.png'}
                    width={20}
                    height={20}
                    alt={title + " 아이콘"} />
                <div className={styles.productTitle}>{title}</div>
    ...
    return (
        <div className={styles.recommendList}>
            <div className={styles.monitorDiv}>{renderMonitors(monitor, "모니터")}</div>
