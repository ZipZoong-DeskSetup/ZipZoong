        ...
        if (imageList.length <= 4) {
            return imageList.map((item, index) => (
                <li key={index} className={styles.carouselItem}>
                    ...
                    <div className={styles.carouselModel}>
                        {item.model}
                    </div>
        ...
        <div>
            <div className={styles.recommendTotal}>
                <div className={styles.carouselContainer}>
                    <button onClick={prev} disabled={currIndex === 0}><IoMdArrowDropleft className={styles.prevBtn} /></button>
                    <ul className={styles.carouselList}>
                        {getCurrentImages()}
                    </ul>
                    <button onClick={next} disabled={isNextDisabled}><IoMdArrowDropright className={styles.prevBtn} /></button>
                </div>
